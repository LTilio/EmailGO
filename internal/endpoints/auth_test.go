...
func Test_Auth_WhenAuthorizationIsMissing_ReturnError(t *testing.T) {
	...
	nextHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Error("nest handler should not be called")
	})
	...
	assert.Contains(res.Body.String(), "request does not contains an authorization header")
}
...
func Test_Auth_WhenAuthorizationIsvalid_CallNextHandler(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "/", nil)
	req.Header.Add("Authorization", "Beared valid-token")
	res := httptest.NewRecorder()

	handlerFunc.ServeHTTP(res, req)

	assert.Equal(http.StatusOK, res.Code)
	assert.Equal(email, emailExpected)
}
